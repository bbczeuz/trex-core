...
class STLS1(object):
    ...
    def __init__ (self):
        ...
        self.vlan_low = 3000
        self.vlan_high = self.vlan_low + 9

    def fill_l2_self(self, direction):
        """
        Send packets with physical interface MACs
        """
        base_pkt =  Ether() / Dot1Q(vlan=self.vlan_low) / IP(src="10.0.0.1",dst="20.0.0.1")/UDP(dport=direction,sport=1025)/"self"
    ...
    def fill_l2_forwarding_table(self, direction, port_id):
        ...
        """
        if direction == 0:
            base_pkt =  Ether(src=self.mac_ab) / /Dot1Q(vlan=self.vlan_low) / IP(src="13.0.0.1", dst="23.0.0.1") / UDP(dport=direction, sport=port_id)/"load"
        else:
            base_pkt =  Ether(src=self.mac_ba) / Dot1Q(vlan=self.vlan_low) / IP(src="14.0.0.1", dst="24.0.0.1") / UDP(dport=direction, sport=port_id)/"load"
